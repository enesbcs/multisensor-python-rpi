...
#        ll = oBacklight.get_level()
#        if (ll < 6):
#         ll = oBacklight.get_previous_level()
#         if ll<6:
#          ll = 5
#        oBacklight.set_level(ll+20)
...
#     print("FM:",frontmotionstatus)
#     print("Front PIR det",frontmotionstatus)
#     if (nv == 1) and (time.time()-frontmotionstatus[1]>10):
#      print("Front motion")
#      oBacklight.set_level_light_compensated(65535)
#      lastmove = time.time()
#     else:
...
#        ds = "{0.year}-{0.month}-{0.day}".format(now)
#        ds = str(now.year) + " " + format(now.month,'02') + "/" + format(now.day,'02') 
...
def sensors():
  ...
  if sCPU.isValueFinal():
    ...
    msg = domomsg.format(ClockConfig.IDX_PITMP, 0, str(ctmp) )
    mqttPublish(msg)
    fl = sensorarr[2]
    wx = fl.findChild(QtGui.QLabel, "wx2")
    wx.setText(str(ctmp)+ u'°C')
...
#    mqttinit = True # DEBUG FORCE MODE!
    ...
    sCPU   = CPUThermal(0,ClockConfig.tempdelaysec)
